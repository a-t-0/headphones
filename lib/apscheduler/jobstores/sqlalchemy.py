...
class SQLAlchemyJobStore(BaseJobStore):
    ...
    def __init__(
        ...
        self.jobs_t = Table(
            ...
            metadata,
            Column(
                "id", Unicode(191, _warn_on_bytestring=False), primary_key=True
            ),
    ...
    def get_next_run_time(self):
        selectable = (
            ...
            .where(self.jobs_t.c.next_run_time != None)
    ...
    def update_job(self, job):
        update = (
            ...
            .values(
                **{
                    "next_run_time": datetime_to_utc_timestamp(
                        job.next_run_time
                    ),
                    "job_state": pickle.dumps(
                        job.__getstate__(), self.pickle_protocol
    ...
    def _get_jobs(self, *conditions):
        jobs = []
        selectable = select(
            [self.jobs_t.c.id, self.jobs_t.c.job_state]
        ).order_by(self.jobs_t.c.next_run_time)
        selectable = (
            selectable.where(*conditions) if conditions else selectable
        )
        failed_job_ids = set()
