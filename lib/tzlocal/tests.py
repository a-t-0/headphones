...
class TzLocalTests(unittest.TestCase):
    def test_env(self):
        ...
        tz_local = tzlocal.unix._tz_from_env(
            ":" + os.path.join(local_path, "test_data", "Harare")
        )
        ...
        self.assertRaises(
            pytz.UnknownTimeZoneError, tzlocal.unix._tz_from_env, "GMT+03:00"
        )
    ...
    def test_timezone(self):
        ...
        tz = tzlocal.unix._get_localzone(
            _root=os.path.join(local_path, "test_data", "timezone")
        )
    ...
    def test_zone_setting(self):
        ...
        local_path = os.path.split(__file__)[0]
        tz = tzlocal.unix._get_localzone(
            _root=os.path.join(local_path, "test_data", "zone_setting")
        )
        self.assertEqual(tz.zone, "Africa/Harare")
    ...
    def test_timezone_setting(self):
        ...
        local_path = os.path.split(__file__)[0]
        tz = tzlocal.unix._get_localzone(
            _root=os.path.join(local_path, "test_data", "timezone_setting")
        )
        self.assertEqual(tz.zone, "Africa/Harare")
