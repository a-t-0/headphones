...
class DropPrivileges(SimplePlugin):
    ...
    def uid(self, val):
        if val is not None:
            if pwd is None:
                self.bus.log(
                    "pwd module not available; ignoring uid.", level=30
                )
                val = None
    ...
    def gid(self, val):
        if val is not None:
            if grp is None:
                self.bus.log(
                    "grp module not available; ignoring gid.", level=30
                )
                val = None
            elif isinstance(val, text_or_bytes):
    ...
    def umask(self, val):
        if val is not None:
            try:
                ...
                self.bus.log(
                    "umask function not available; ignoring umask.", level=30
                )
                val = None
        self._umask = val
...
class Daemonizer(SimplePlugin):
    ...
    def __init__(
        self, bus, stdin="/dev/null", stdout="/dev/null", stderr="/dev/null"
    ):
        SimplePlugin.__init__(self, bus)
        self.stdin = stdin
        self.stdout = stdout
