...
def check_list(list, ignore=0):
    ...
    for i in range(first_none, len(list2)):
        if list2[i]:
            raise ValueError(
                "non-None entry after None entry in list at index {0}".format(
                    i
                )
            )
    ...
    def __init__(self, path):
        ...
        def track_parser(start_line):
            ...
            if not search_result:
                raise ValueError(
                    "inconsistent CUE sheet, TRACK expected at line {0}".format(
                        line_index + 1
                    )
                )
            ...
            while not next_track:
                if line_index < len(c):
                    ...
                    if artist_search:
                        ...
                        raise ValueError(
                            "unknown entry in track error, line {0}".format(
                                line_index + 1
                            )
                        )
                else:
...
def split(albumpath):
    ...
    if "xld" in splitter:
        ...
        if not base_dir.filter("MetaFile"):
            raise ValueError(
                "Cue Meta file {0} missing!".format(ALBUM_META_FILE_NAME)
            )
        else:
