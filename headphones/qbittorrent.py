...
class qbittorrentclient(object):
    ...
    def __init__(
        ...
        try:
            ...
            if login_text:
                logger.warning(
                    "Could not login to qBittorrent v2 api, check credentials: %s",
                    login_text,
                )
            self.version = 2
        except Exception as e:
            logger.warning(
                "Error with qBittorrent v2 api, check settings or update, will try v1: %s"
                % e
            )
    ...
    def _get_sid(self, base_url, username, password):
        ...
        for cookie in self.cookiejar:
            logger.debug(
                "login cookie: " + cookie.name + ", value: " + cookie.value
            )
        return
    ...
    def _command(self, command, args=None, content_type=None, files=None):
        ...
        try:
            ...
            logger.debug(
                "QBitTorrent webUI raised the following error: %s" % str(err)
            )
            return None, None
    ...
    def start(self, hash):
        ...
        return self._command(
            "command/resume", args, "application/x-www-form-urlencoded"
        )
    ...
    def pause(self, hash):
        ...
        args = {"hash": hash}
        return self._command(
            "command/pause", args, "application/x-www-form-urlencoded"
        )
    ...
    def setprio(self, hash, priority):
        ...
        return self._command(
            "command/setFilePrio", args, "application/x-www-form-urlencoded"
        )
    ...
    def remove(self, hash, remove_data=False):
        ...
        return self._command(
            command, args, "application/x-www-form-urlencoded"
        )
...
def removeTorrent(hash, remove_data=False):
    ...
    for torrent in torrentlist:
        if torrent["hash"].lower() == hash.lower():
            if (
                torrent["ratio"] >= torrent["ratio_limit"]
                ...
                if qbclient.version == 2:
                    if remove_data:
                        ...
                        logger.info(
                            "%s has finished seeding, removing torrent"
                            % torrent["name"]
                        )
...
def addTorrent(link):
    ...
    if qbclient.version == 2:
        ...
        return qbclient._command(
            "command/download", args, "multipart/form-data"
        )
...
def addFile(data):
    ...
    qbclient = qbittorrentclient()
    if qbclient.version == 2:
        return qbclient.qb.download_from_file(
            data,
            savepath=headphones.CONFIG.DOWNLOAD_TORRENT_DIR,
            category=headphones.CONFIG.QBITTORRENT_LABEL,
        )
