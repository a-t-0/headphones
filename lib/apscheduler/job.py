...
class Job(object):
    ...
    def _modify(self, **changes):
        ...
        if "func" in changes or "args" in changes or "kwargs" in changes:
            ...
            kwargs = (
                changes.pop("kwargs") if "kwargs" in changes else self.kwargs
            )
    ...
    def __getstate__(self):
        ...
        if not self.func_ref:
            raise ValueError(
                "This Job cannot be serialized since the reference to its callable (%r) could not be "
    ...
    def __repr__(self):
        return "<Job (id=%s name=%s)>" % (
            repr_escape(self.id),
            repr_escape(self.name),
        )

    def __str__(self):
        return "%s (trigger: %s, next run at: %s)" % (
            repr_escape(self.name),
            repr_escape(str(self.trigger)),
