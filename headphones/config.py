...
_CONFIG_DEFINITIONS = {
    ...
    "OPEN_MAGNET_LINKS": (
        ...
        "General",
        0,
    ),  # 0: Ignore, 1: Open, 2: Convert, 3: Embed (rtorrent)
...
class Config:
    ...
    def check_setting(self, key):
        """Cast any value in the config to the right type or use the
        default."""
    ...
    def write(self):
        """Make a copy of the stored config and write it to the configured
        file."""
    ...
    def get_extra_torznabs(self):
        ...
        extra_torznabs = list(
            zip(
                *[
                    itertools.islice(self.EXTRA_TORZNABS, i, None, 4)
                    for i in range(4)
                ]
            )
        )
    ...
    def __getattr__(self, name):
        """Returns something from the ini unless it is a real property of the
        configuration object or is not all caps."""
    ...
    def _upgrade(self):
        ...
        Although this is kind of a dumb way to do it because it doesn't
        handle multi-step upgrades
        """
        if self.CONFIG_VERSION == "2":
            ...
            if self.ENCODERFOLDER:
                self.ENCODER_PATH = os.path.join(
                    self.ENCODERFOLDER, self.ENCODER
                )
        ...
        if self.CONFIG_VERSION == "5":
            ...
            if self.EXTRA_TORZNABS:
                extra_torznabs = list(
                    zip(
                        *[
                            itertools.islice(self.EXTRA_TORZNABS, i, None, 3)
                            for i in range(3)
                        ]
                    )
                )
