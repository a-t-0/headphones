...
from .unittestcompat import TestArgs, TestCase


class ConfigApiTest(TestCase):
    """Common tests for headphones.Config.

    Common tests for headphones.Config This test suite guarantees, that
    external API of the Config class conforms all expectations of other
    modules.
    """
    ...
    def setUp(self):
        ...
        self.config_module_mock_patcher = mock.patch(
            "headphones.config.ConfigObj", name="ConfigObjModuleMock"
        )
    ...
    def test_check_setting(
        self, setting_name, expected_return, expected_instance
    ):
    ...
    def test_write(self):
        ...
        option_name_not_from_definitions = (
            "some_invalid_option_with_super_uniq1_name"
        )
        option_name_not_from_definitions_value = 1
        old_conf_mock["asdf"] = {
            option_name_not_from_definitions: option_name_not_from_definitions_value
        }
        ...
        new_patcher = mock.patch(
            "headphones.config.ConfigObj",
            name="NEW_ConfigObjModuleMock_FOR_WRITE",
        )

        new_conf_module_mock = new_patcher.start()
        new_conf_mock = new_conf_module_mock.return_value = MagicMock()
        cf.write()
        new_patcher.stop()

        # assertions:
        self.assertFalse(
            old_conf_mock.write.called, "write not called for old config"
        )
        self.assertTrue(
            new_conf_mock.write.called, "write called for new config"
        )
        self.assertEqual(new_conf_mock.filename, path)

        new_conf_mock["General"].__setitem__.assert_any_call(
            "download_dir", ""
        )
        # from 3.5... new_conf_mock['asdf'].__setitem__.assert_not_called('download_dir', '')
        new_conf_mock["asdf"].__setitem__.assert_any_call(
            option_name_not_from_definitions,
    ...
    def test__setattr__ExplicitSetUsingDotNotation(self):
        ...
        # assertions:
        self.assertEqual(
            self.config_mock["General"]["album_art_format"], "Apple"
        )
    ...
    @TestArgs(
        ...
        (
            [1, 2, 3, "Aaa", "Bbba", "Ccccc"],
            [(1, 2, 3), ("Aaa", "Bbba", "Ccccc")],
        ),
        (
            [1, 2, 3, "Aaa", "Bbba", "Ccccc", "Ddddda"],
            [(1, 2, 3), ("Aaa", "Bbba", "Ccccc")],
        ),
    )
