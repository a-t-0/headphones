...
class Rutracker:
    ...
    def still_logged_in(self, html):
        if (
            not html
            or 'action="https://rutracker.org/forum/login.php">' in html
        ):
            return False
        else:
            return True
    ...
    def has_bb_session_cookie(self, response):
        ...
        return next(
            ("bb_session" in list(r.cookies.keys()) for r in response.history),
            False,
        )
    ...
    def search(self, searchurl):
        ...
        try:
            ...
            r = self.session.get(
                url=searchurl, headers=headers, timeout=self.timeout
            )
            ...
            for item in zip(
                i.find_all(class_="hl-tags"),
                i.find_all(class_="dl-stub"),
                ...
                if size < self.maxsize and minimumseeders < int(seeds):
                    logger.info(f"Found {title}. Size: {size_formatted}")
                    # Torrent topic page
                    torrent_id = dict(
                        [
                            part.split("=")
                            for part in urlparse(url)[4].split("&")
                        ]
                    )["t"]
                    topicurl = (
                    ...
                    rulist.append(
                        Result(
                            title, size, url, "rutracker.org", "torrent", True
                        )
                    )
                else:
                    logger.info(
            ...
            logger.error(
                "An unknown error occurred in the rutracker parser: %s" % e
            )
            return None
    ...
    def get_torrent_data(self, url):
        ...
        torrent_id = dict(
            [part.split("=") for part in urlparse(url)[4].split("&")]
        )["t"]
    ...
    def utorrent_add_file(self, data):
        ...
        url = base_url + "/gui/"
        self.session.auth = (
            headphones.CONFIG.UTORRENT_USERNAME,
            headphones.CONFIG.UTORRENT_PASSWORD,
        )
    ...
    def qbittorrent_add_file(self, data):
        ...
        try:
            self.session.post(
                url,
                data={
                    "username": headphones.CONFIG.QBITTORRENT_USERNAME,
