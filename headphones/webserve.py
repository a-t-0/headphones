...
def serve_template(templatename, **kwargs):
    ...
    template_dir = os.path.join(
        str(interface_dir), headphones.CONFIG.INTERFACE
    )
...
class WebInterface:
    ...
    def home(self):
        ...
        return serve_template(
            templatename="index.html", title="Home", artists=artists
        )
    ...
    def albumPage(self, AlbumID):
        ...
        tracks = myDB.select(
            "SELECT * from tracks WHERE AlbumID=? ORDER BY CAST(TrackNumber AS INTEGER)",
            [AlbumID],
        )
        description = myDB.action(
            "SELECT * from descriptions WHERE ReleaseGroupID=?", [AlbumID]
        ).fetchone()
    ...
    def addArtist(self, artistid):
        thread = threading.Thread(
            target=importer.addArtisttoDB, args=[artistid]
        )
        thread.start()
        thread.join(1)
    ...
    def getExtras(self, ArtistID, newstyle=False, **kwargs):
        ...
        newValueDict = {"IncludeExtras": 1, "Extras": extras}
        myDB.upsert("artists", newValueDict, controlValueDict)
        thread = threading.Thread(
            target=importer.addArtisttoDB, args=[ArtistID, True, False]
        )
        thread.start()
        thread.join(1)
        raise cherrypy.HTTPRedirect("artistPage?ArtistID=%s" % ArtistID)
    ...
    def removeExtras(self, ArtistID, ArtistName):
        ...
        for album in extraalbums:
            myDB.action(
                "DELETE from tracks WHERE ArtistID=? AND AlbumID=?",
                [ArtistID, album["AlbumID"]],
            )
            myDB.action(
                "DELETE from albums WHERE ArtistID=? AND AlbumID=?",
                [ArtistID, album["AlbumID"]],
            )
            myDB.action(
                "DELETE from allalbums WHERE ArtistID=? AND AlbumID=?",
                [ArtistID, album["AlbumID"]],
            )
            myDB.action(
                "DELETE from alltracks WHERE ArtistID=? AND AlbumID=?",
                [ArtistID, album["AlbumID"]],
            )
            myDB.action(
                "DELETE from releases WHERE ReleaseGroupID=?",
                [album["AlbumID"]],
            )
    ...
    def removeArtist(self, ArtistID):
        ...
        for rgid in rgids:
            ...
            myDB.action(
                "DELETE from releases WHERE ReleaseGroupID=?", [albumid]
            )
            myDB.action("DELETE from have WHERE Matched=?", [albumid])
            c.remove_from_cache(AlbumID=albumid)
            myDB.action(
                "DELETE from descriptions WHERE ReleaseGroupID=?", [albumid]
            )
    ...
    def scanArtist(self, ArtistID):
        ...
        myDB = db.DBConnection()
        artist_name = myDB.select(
            "SELECT DISTINCT ArtistName FROM artists WHERE ArtistID=?",
            [ArtistID],
        )[0][0]
        ...
        folder_format = re.findall(r"(.*?[Aa]rtist?)\.*", full_folder_format)[
            0
        ]
        ...
        folder = pattern_substitute(
            folder_format.strip(), values, normalize=True
        )
        ...
        try:
            for dir in dirs:
                ...
                if not os.path.isdir(
                    artistfolder.encode(headphones.SYS_ENCODING)
                ):
    ...
    def refreshArtist(self, ArtistID):
        thread = threading.Thread(
            target=importer.addArtisttoDB, args=[ArtistID, False, True]
        )
        thread.start()
        thread.join(1)
        raise cherrypy.HTTPRedirect("artistPage?ArtistID=%s" % ArtistID)
        ...
        for mbid in args:
            ...
            if ArtistID:
                ...
                ArtistIDT = myDB.action(
                    "SELECT ArtistID FROM albums WHERE AlbumID=?", [mbid]
                ).fetchone()[0]
            myDB.action(
    ...
    def addArtists(self, action=None, **args):
        if action == "add":
            threading.Thread(
                target=importer.artistlist_to_mbids, args=[args, True]
            ).start()
        if action == "ignore":
            myDB = db.DBConnection()
            for artist in args:
                myDB.action(
                    "DELETE FROM newartists WHERE ArtistName=?",
                ...
                logger.info(
                    "Artist %s removed from new artist list and set to ignored"
                    % artist
                )
        raise cherrypy.HTTPRedirect("home")
    ...
    @cherrypy.expose
    def queueAlbum(
        self, AlbumID, ArtistID=None, new=False, redirect=None, lossless=False
    ):
    ...
    def choose_specific_download(self, AlbumID):
        results = (
            searcher.searchforalbum(AlbumID, choose_specific_download=True)
            or []
        )
    ...
    @cherrypy.expose
    @cherrypy.tools.json_out()
    def download_specific_release(
        self, AlbumID, title, size, url, provider, kind, **kwargs
    ):
        # Handle situations where the torrent url contains arguments that are parsed
        if kwargs:
            url = (
                parse.quote(url, safe=":?/=&") + "&" + parse.urlencode(kwargs)
            )
        try:
    ...
    def deleteAlbum(self, AlbumID, ArtistID=None):
        ...
        myDB.action("DELETE from have WHERE Matched=?", [AlbumID])
        album = myDB.action(
            "SELECT ArtistID, ArtistName, AlbumTitle from albums where AlbumID=?",
            [AlbumID],
        ).fetchone()
        if album:
            ArtistID = album["ArtistID"]
            myDB.action(
                "DELETE from have WHERE ArtistName=? AND AlbumTitle=?",
    ...
    def switchAlbum(self, AlbumID, ReleaseID):
        """Take the values from allalbums/alltracks (based on the ReleaseID)
        and swap it into the album & track tables."""
    ...
    def manageArtists(self):
        ...
        return serve_template(
            templatename="manageartists.html",
    ...
    def manageAlbums(self, Status=None):
        ...
        return serve_template(
            templatename="managealbums.html",
            title="Manage Albums",
            albums=albums,
        )
    ...
    def manageNew(self):
        ...
        return serve_template(
            templatename="managenew.html",
            title="Manage New Artists",
    ...
    def manageUnmatched(self):
        ...
        for albums in have_albums:
            ...
            if (
                albums["ArtistName"]
                and albums["AlbumTitle"]
                and albums["TrackTitle"]
            ):
        ...
        unmatchedalbums = [
            d
            for d in have_album_dictionary
            if (
                clean_name(d["ArtistName"]).lower(),
                clean_name(d["AlbumTitle"]).lower(),
            )
        ...
        return serve_template(
            templatename="manageunmatched.html",
            title="Manage Unmatched Items",
    ...
    def markUnmatched(
        ...
        if action == "ignoreArtist":
            ...
            if new_artist_clean != existing_artist_clean:
                ...
                logger.info(
                    "Manual matching yielded %s new matches for Artist: %s"
                ...
                logger.info(
                    "Artist %s already named appropriately; nothing to modify"
                    % existing_artist
                )
            ...
            existing_clean_string = (
                existing_artist_clean + " " + existing_album_clean
            )
            ...
            if existing_clean_string != new_clean_string:
                ...
                logger.info(
                    "Manual matching yielded %s new matches for Artist: %s / Album: %s"
    ...
    def manageManual(self):
        ...
        for albums in manualalbums:
            if (
                ...
                if (
                    albums["Matched"] == "Ignored"
                    or albums["Matched"] == "Manual"
                    or albums["CleanName"] != original_clean
                ):
                    if albums["Matched"] == "Ignored":
                        album_status = "Ignored"
                    elif (
                        albums["Matched"] == "Manual"
                        or albums["CleanName"] != original_clean
                    ):
        ...
        return serve_template(
            templatename="managemanual.html",
            title="Manage Manual Items",
            manualalbums=manual_albums_sorted,
        )

    @cherrypy.expose
    def markManual(
        self, action=None, existing_artist=None, existing_album=None
    ):
        myDB = db.DBConnection()
        if action == "unignoreArtist":
            artist = existing_artist
            myDB.action(
                'UPDATE have SET Matched="Failed" WHERE ArtistName=? AND Matched="Ignored"',
                [artist],
            )
            logger.info(
            ...
            for tracks in update_clean:
                ...
                if tracks["CleanName"] != original_clean:
                    artist_id_check = myDB.action(
                        "SELECT ArtistID FROM tracks WHERE CleanName = ?",
                        [tracks["CleanName"]],
                    ).fetchone()
                    if artist_id_check:
            ...
            for tracks in update_clean:
                ...
                if tracks["CleanName"] != original_clean:
                    album_id_check = myDB.action(
                        "SELECT AlbumID FROM tracks WHERE CleanName = ?",
                        [tracks["CleanName"]],
                    ).fetchone()
                    if album_id_check:
    ...
    def markArtists(self, action=None, **args):
        ...
        if len(artistsToAdd) > 0:
            ...
            threading.Thread(
                target=importer.addArtistIDListToDB, args=[artistsToAdd]
            ).start()
        raise cherrypy.HTTPRedirect("home")
    ...
    def importLastFMTag(self, tag, limit):
        threading.Thread(
            target=lastfm.getTagTopArtists, args=(tag, limit)
        ).start()
        raise cherrypy.HTTPRedirect("home")
    ...
    def musicScan(self, path, scan=0, redirect=None, autoadd=0, libraryscan=0):
        ...
        if scan:
            try:
                threading.Thread(
                    target=librarysync.libraryScan, kwargs=params
                ).start()
            except Exception as e:
                logger.error("Unable to complete the scan: %s" % e)
        if redirect:
    ...
    def forcePostProcess(
        self, dir=None, album_dir=None, keep_original_folder=False
    ):
        from headphones import postprocessor

        threading.Thread(
            target=postprocessor.forcePostProcess,
            kwargs={
                "dir": dir,
                "album_dir": album_dir,
    ...
    def history(self):
        ...
        return serve_template(
            templatename="history.html", title="History", history=history
        )

    @cherrypy.expose
    def logs(self):
        return serve_template(
            templatename="logs.html", title="Log", lineList=headphones.LOG_LIST
        )
    ...
    def getLog(
        ...
        **kwargs,
        ...
        if sSearch == "":
            ...
            filtered = [
                row
                for row in headphones.LOG_LIST
                for column in row
                if sSearch.lower() in column.lower()
            ]
    ...
    def getArtistjson(self, ArtistID, **kwargs):
        ...
        return {"ArtistName": artist["ArtistName"], "Status": artist["Status"]}
    ...
    def clearhistory(self, type=None, date_added=None, title=None):
        ...
        if type:
            if type == "all":
                ...
                myDB.action(
                    'DELETE from snatched WHERE Status NOT LIKE "Seed%"'
                )
            else:
    ...
    def config(self):
        ...
        config = {
            ...
            "nzb_downloader_sabnzbd": radio(
                headphones.CONFIG.NZB_DOWNLOADER, 0
            ),
            "nzb_downloader_nzbget": radio(
                headphones.CONFIG.NZB_DOWNLOADER, 1
            ),
            "nzb_downloader_blackhole": radio(
                headphones.CONFIG.NZB_DOWNLOADER, 2
            ),
            "torrent_downloader_blackhole": radio(
                headphones.CONFIG.TORRENT_DOWNLOADER, 0
            ),
            "torrent_downloader_transmission": radio(
                headphones.CONFIG.TORRENT_DOWNLOADER, 1
            ),
            "torrent_downloader_utorrent": radio(
                headphones.CONFIG.TORRENT_DOWNLOADER, 2
            ),
            "torrent_downloader_deluge": radio(
                headphones.CONFIG.TORRENT_DOWNLOADER, 3
            ),
            "torrent_downloader_qbittorrent": radio(
                headphones.CONFIG.TORRENT_DOWNLOADER, 4
            ),
            ...
            "ignore_clean_releases": checked(
                headphones.CONFIG.IGNORE_CLEAN_RELEASES
            ),
            ...
            "rename_single_disc_ignore": checked(
            ...
            "replace_existing_folders": checked(
                headphones.CONFIG.REPLACE_EXISTING_FOLDERS
            ),
            "keep_original_folder": checked(
                headphones.CONFIG.KEEP_ORIGINAL_FOLDER
            ),
            ...
            "official_releases_only": checked(
                headphones.CONFIG.OFFICIAL_RELEASES_ONLY
            ),
            "wait_until_release_date": checked(
            ...
            "autowant_manually_added": checked(
                headphones.CONFIG.AUTOWANT_MANUALLY_ADDED
            ),
            "do_not_process_unmatched": checked(
                headphones.CONFIG.DO_NOT_PROCESS_UNMATCHED
            ),
            "keep_torrent_files": checked(
                headphones.CONFIG.KEEP_TORRENT_FILES
            ),
            ...
            "encoder_multicore_count": int(
                headphones.CONFIG.ENCODER_MULTICORE_COUNT
            ),
            "delete_lossless_files": checked(
                headphones.CONFIG.DELETE_LOSSLESS_FILES
            ),
            ...
            "pushbullet_enabled": checked(
                headphones.CONFIG.PUSHBULLET_ENABLED
            ),
            "pushbullet_onsnatch": checked(
                headphones.CONFIG.PUSHBULLET_ONSNATCH
            ),
            ...
            "osx_notify_enabled": checked(
                headphones.CONFIG.OSX_NOTIFY_ENABLED
            ),
            "osx_notify_onsnatch": checked(
                headphones.CONFIG.OSX_NOTIFY_ONSNATCH
            ),
            ...
            "songkick_filter_enabled": checked(
                headphones.CONFIG.SONGKICK_FILTER_ENABLED
            ),
        ...
        extra_munges = {"dj-mix": "dj_mix", "mixtape/street": "mixtape_street"}

        extras_list = [
            extra_munges.get(x, x) for x in headphones.POSSIBLE_EXTRAS
        ]
        ...
        return serve_template(
            templatename="config.html", title="Settings", config=config
        )
    ...
    def configUpdate(self, **kwargs):
        ...
        checked_configs = [
            ...
            "use_orpheus",
            "use_redacted",
            "redacted_use_fltoken",
            "preferred_bitrate_allow_lossless",
            "detect_bitrate",
            "ignore_clean_releases",
            "freeze_db",
            "cue_split",
            "move_files",
            "rename_files",
            "rename_single_disc_ignore",
            "correct_metadata",
            "cleanup_files",
            "keep_nfo",
            "add_album_art",
            ...
            "autowant_upcoming",
            "autowant_all",
            "autowant_manually_added",
            "do_not_process_unmatched",
            "keep_torrent_files",
            "music_encoder",
        ...
        for plain_config, use_config in [
            (x[4:], x) for x in kwargs if x.startswith("use_")
        ]:
        ...
        for kwarg in [x for x in kwargs if x.startswith("torznab_host")]:
            ...
            if len(torznab_number):
                ...
                for key in [
                    ...
                    if key in kwargs:
                        del kwargs[key]
                extra_torznabs.append(
                    (torznab_host, torznab_api, torznab_ratio, torznab_enabled)
                )
        ...
        # Sanity checking
        if (
            headphones.CONFIG.SEARCH_INTERVAL
            and headphones.CONFIG.SEARCH_INTERVAL < 360
        ):
    ...
    def extras(self):
        ...
        return serve_template(
            templatename="extras.html", title="Extras", cloudlist=cloudlist
        )

    @cherrypy.expose
    def addReleaseById(self, rid, rgid=None):
        threading.Thread(
            target=importer.addReleaseById, args=[rid, rgid]
        ).start()
        if rgid:
    ...
    def getImageLinks(self, ArtistID=None, AlbumID=None):
        ...
        if AlbumID and not image_dict:
            image_url = (
                "https://coverartarchive.org/release/%s/front-500.jpg"
                % AlbumID
            )
            thumb_url = (
                "https://coverartarchive.org/release/%s/front-250.jpg"
                % AlbumID
            )
            image_dict = {"artwork": image_url, "thumbnail": thumb_url}
        elif AlbumID and (
            not image_dict["artwork"] or not image_dict["thumbnail"]
        ):
    ...
    def testTwitter(self):
        ...
        if result:
            return (
                "Tweet successful, check your twitter to make sure it worked"
            )
        else:
