...
class ResponseEncoder:
    ...
    def find_acceptable_charset(self):
        ...
        if self.debug:
            cherrypy.log(
                "response.stream %r" % response.stream, "TOOLS.ENCODE"
            )
        if response.stream:
        ...
        if self.encoding is not None:
            ...
            if self.debug:
                cherrypy.log(
                    "Specified encoding %r" % encoding, "TOOLS.ENCODE"
                )
            if (not charsets) or "*" in charsets or encoding in charsets:
                if self.debug:
                    cherrypy.log(
                        "Attempting encoding %r" % encoding, "TOOLS.ENCODE"
                    )
                if encoder(encoding):
                    return encoding
        else:
    ...
    def __call__(self, *args, **kwargs):
        ...
        if self.debug:
            cherrypy.log(
                "Content-Type: %r" % [str(h) for h in ct], "TOOLS.ENCODE"
            )
        if ct and self.add_charset:
            ...
            if self.text_only:
                ...
                if self.debug:
                    cherrypy.log(
                        "Finding because not text_only", "TOOLS.ENCODE"
                    )
                do_find = True
            ...
            if do_find:
                ...
                if self.debug:
                    cherrypy.log(
                        "Setting Content-Type %s" % ct, "TOOLS.ENCODE"
                    )
    ...
    for coding in acceptable:
        ...
        if coding.value in ("gzip", "x-gzip"):
            if coding.qvalue == 0:
                if debug:
                    cherrypy.log(
                        "Zero gzip qvalue: %s" % coding, context="TOOLS.GZIP"
                    )
                return
