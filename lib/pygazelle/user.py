...
class User(object):
    ...
    def __init__(self, id, parent_api):
        ...
        self.parent_api.cached_users[
            self.id
        ] = self  # add self to cache of known User objects
    ...
    def set_index_data(self, index_json_response):
        ...
        if self.id != index_json_response["id"]:
            raise InvalidUserException(
                "Tried to update non-logged-in User's information from 'index' API call."
                + " Should be %s, got %s"
                % (self.id, index_json_response["id"])
            )
        ...
        if self.stats:
            self.stats = dict(
                ...
                + list(index_json_response["userstats"].items())
            )  # merge in new info
        else:
    ...
    def set_user_data(self, user_json_response):
        ...
        if self.username and self.username != user_json_response["username"]:
            raise InvalidUserException(
                "Tried to update a user's information from a 'user' API call with a different username."
                + " Should be %s, got %s"
        ...
        if self.stats:
            self.stats = dict(
                list(self.stats.items())
                + list(user_json_response["stats"].items())
            )  # merge in new info
        else:
    ...
    def set_search_result_data(self, search_result_item):
        ...
        if self.id != search_result_item["userId"]:
            raise InvalidUserException(
                "Tried to update existing user with another user's search result data (IDs don't match)."
            )
