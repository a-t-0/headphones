...
class MetadataTest(TestCase):
    ...
    def test_metadata_dict_ci(self):
        ...
        self.assertEqual(
            m[key_var.lower()], expected, "same-case lookup " "succes"
        )
    ...
    def test_metadata_dict_cs(self):
        ...
        key_var = "$TitlE"
        m = MetadataDict(
            {key_var.lower(): expected_var.lower(), key_var: expected_var}
        )
        self.assertFalse("$track" in m)
        self.assertTrue("$tITLe" in m, "cross-case lookup with 'in'")
        self.assertEqual(
        ...
        self.assertEqual(
            m[key_var], expected_var, "case-preserving lookup variable"
        )
    ...
    def test_dict_intersect(self):
        ...
        self.assertItemsEqual(
            expected,
            _md._intersect(d1, d2),
            "check dictionary intersection " "is common part indeed",
        )
        del d1["two"]
        expected = {}
        self.assertItemsEqual(
    ...
    def test_album_metadata_with_None(self):
        ...
        mb.add_media_file(f2)
        md = _md.album_metadata(
            "/music/Artist - Album [2002]", row, mb.build()
        )
