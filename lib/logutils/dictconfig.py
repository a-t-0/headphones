            ...
            if incremental:
                ...
                if named_handlers_supported():
                    for name in handlers:
                        if name not in logging._handlers:
                            raise ValueError(
                                "No handler found with " "name %r" % name
                            )
                        else:
                            try:
                ...
                if root:
                    try:
                        ...
                        e = sys.exc_info()[1]
                        raise ValueError(
                            "Unable to configure root " "logger: %s" % e
                        )
            else:
    ...
    def configure_handler(self, config):
        ...
        if formatter:
            try:
                ...
                e = sys.exc_info()[1]
                raise ValueError(
                    "Unable to set formatter " "%r: %s" % (formatter, e)
                )
        ...
        if "()" in config:
            ...
            if (
                issubclass(klass, logging.handlers.MemoryHandler)
                ...
                issubclass(klass, logging.handlers.SMTPHandler)
