...
def _check_filter_and_make_params(
...
def set_useragent(app, version, contact=None):
    ...
    if contact is not None:
        _useragent = "%s/%s python-musicbrainzngs/%s ( %s )" % (
            app,
            version,
            _version,
            contact,
        ...
        _useragent = "%s/%s python-musicbrainzngs/%s" % (
            app,
            version,
            _version,
        )
...
def _do_mb_search(
    entity, query="", fields={}, limit=None, offset=None, strict=False
):
...
def get_releases_by_discid(
    id, includes=[], toc=None, cdstubs=True, media_format=None
):
...
def _do_collection_query(collection, collection_type, limit, offset):
    ...
    return _do_mb_query(
        "collection", "%s/%s" % (collection, collection_type), [], params
    )
