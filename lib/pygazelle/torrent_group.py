...
class TorrentGroup(object):
    ...
    def set_group_data(self, torrent_group_json_response):
        ...
        if self.id != torrent_group_json_response["group"]["id"]:
            raise InvalidTorrentGroupException(
                "Tried to update a TorrentGroup's information from an 'artist' API call with a different id."
                + " Should be %s, got %s"
                % (self.id, torrent_group_json_response["group"]["groupId"])
            )
        ...
        self.music_info["artists"] = [
            self.parent_api.get_artist(artist["id"], artist["name"])
            for artist in self.music_info["artists"]
        ]
        self.music_info["with"] = [
            self.parent_api.get_artist(artist["id"], artist["name"])
        ...
        if "torrents" in torrent_group_json_response:
            ...
            torrent = self.parent_api.get_torrent(
                torrent_group_json_response["torrent"]["id"]
            )
            self.torrents.append(torrent)
    ...
    def set_artist_group_data(self, artist_group_json_response):
        ...
        if self.id != artist_group_json_response["groupId"]:
            raise InvalidTorrentGroupException(
                "Tried to update a TorrentGroup's information from an 'artist' API call with a different id."
                + " Should be %s, got %s"
                % (self.id, artist_group_json_response["groupId"])
            )
        ...
        self.catalogue_number = artist_group_json_response[
    ...
    def set_torrent_search_data(self, search_json_response):
        if self.id != search_json_response["groupId"]:
            raise InvalidTorrentGroupException(
                "Tried to update a TorrentGroup's information from an 'browse'/search API call with a different id."
                + " Should be %s, got %s"
                % (self.id, search_json_response["groupId"])
            )
        ...
        if "torrentId" in list(search_json_response.keys()):
            search_json_response["torrents"] = [
                {"torrentId": search_json_response["torrentId"]}
            ]
