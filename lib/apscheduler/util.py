...
def astimezone(obj):
    ...
    if obj is not None:
        raise TypeError(
            "Expected tzinfo, got %s instead" % obj.__class__.__name__
        )
...
def datetime_ceil(dateval):
    ...
    if dateval.microsecond > 0:
        return dateval + timedelta(
            seconds=1, microseconds=-dateval.microsecond
        )
    return dateval
...
def check_callable_args(func, args, kwargs):
    ...
    pos_kwargs_conflicts = (
        []
    )  # parameters that have a match in both args and kwargs
    ...
    unsatisfied_kwargs = (
        []
    )  # keyword-only arguments that don't have a match in kwargs
    unmatched_args = list(
        args
    )  # args that didn't match any of the parameters in the signature
    unmatched_kwargs = list(
        kwargs
    )  # kwargs that didn't match any of the parameters in the signature
    has_varargs = (
        has_var_kwargs
    ) = False  # indicates if the signature defines *args and **kwargs respectively
    ...
    if signature:
        ...
        for arg, default in six.moves.zip_longest(
            argspec_args, argspec.defaults or (), fillvalue=undefined
        ):
