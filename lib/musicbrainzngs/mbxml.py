...
def parse_elements(valid_els, inner_els, element):
    ...
    for sub in element:
        ...
        if t in valid_els:
            ...
            _log.info(
                "in <%s>, uncaught <%s>", fixtag(element.tag, NS_MAP)[0], t
            )
    return result
...
def parse_relation(relation):
    ...
    result.update(
        parse_elements(
            ["target-credit"],
            {"attribute-list": parse_relation_attribute_list},
            relation,
        )
    )
...
def parse_track(track):
    ...
    if (
        "artist-credit" in result.get("recording", {})
...
def make_tag_request(**kwargs):
    ...
    for entity_type in [
        ...
        if entity_tags is not None:
            e_list = ET.SubElement(
                root, "{%s}%s-list" % (NS, entity_type.replace("_", "-"))
            )
            for e, tags in entity_tags.items():
                e_xml = ET.SubElement(
                    e_list, "{%s}%s" % (NS, entity_type.replace("_", "-"))
                )
                e_xml.set("{%s}id" % NS, e)
    ...
    if kwargs.keys():
        raise TypeError(
            "make_tag_request() got an unexpected keyword argument '%s'"
            % kwargs.popitem()[0]
        )
...
def make_rating_request(**kwargs):
    ...
    if kwargs.keys():
        raise TypeError(
            "make_rating_request() got an unexpected keyword argument '%s'"
            % kwargs.popitem()[0]
        )
