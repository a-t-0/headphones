from headphones.helpers import clean_name, is_valid_date
...
class HelpersTest(TestCase):
    def test_clean_name(self):
        ...
        cases = {
            ...
            "Orquesta de la Luz (オルケスタ・デ・ラ・ルス)": "Orquesta de la Luz オルケスタ デ ラ ルス",
        }
        for first, second in cases.items():
            nf = clean_name(first).lower()
            ns = clean_name(second).lower()
            self.assertEqual(
                nf, ns, "check cleaning of case (%s," "%s)" % (nf, ns)
            )
    ...
    def test_is_valid_date(date):
        test_cases = [
            (
                "2021-11-12",
                True,
                "check is_valid_date returns True for valid date",
            ),
            (None, False, "check is_valid_date returns False for None"),
            (
                "2021-11",
                False,
                "check is_valid_date returns False for incomplete",
            ),
            (
                "2021",
                False,
