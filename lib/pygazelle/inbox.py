class MailboxMessage(object):
    ...
    def __repr__(self):
        return "MailboxMessage ID %s - %s %s %s" % (
            self.id,
            self.subject,
            self.sender_id,
            self.username,
...
class ConversationMessage(object):
    ...
    def __repr__(self):
        return "ConversationMessage ID %s - %s %s" % (
            self.id,
            self.sender_name,
            self.sent_date,
        )
...
class Conversation(object):
    ...
    def update_conv_data(self):
        response = self.parent_api.request(
            action="inbox", type="viewconv", id=self.id
        )
...
class Mailbox(object):
    ...
    def next_page(self):
        ...
        if cur_page < total_pages:
            return Mailbox(
                self.parent_api, self.boxtype, str(cur_page + 1), self.sort
            )
