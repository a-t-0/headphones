...
import headphones
from headphones import db, helpers, logger, mb, metacritic
...
def is_exists(artistid):
    ...
    if any(artistid in x for x in artistlist):
        logger.info(
            artistlist[0][1]
            + " is already in the database. Updating 'have tracks', but not artist information"
        )
        return True
    else:
        return False
    ...
    for artist in artistlist:
        ...
        if not forced:
            bl_artist = myDB.action(
                "SELECT * FROM blacklist WHERE ArtistID=?", [artistid]
            ).fetchone()
...
def addArtisttoDB(artistid, extrasonly=False, forcefull=False, type="artist"):
    ...
    if (
        artist
        and artist.get("artist_name") in blacklisted_special_artist_names
    ):
        logger.warn(
            "Cannot import blocked special purpose artist: %s"
    ...
    if not artist:
        ...
        if dbartist is None:
            newValueDict = {
                "ArtistName": "Fetch failed, try refreshing. (%s)"
                % (artistid),
                "Status": "Active",
            }
        else:
    ...
    for rg in artist["releasegroups"]:
        ...
        rg_exists = myDB.action(
            "SELECT * from albums WHERE AlbumID=?", [rg["id"]]
        ).fetchone()
        ...
        if not forcefull:
            ...
            if new_release_group:
                logger.info(
                    "[%s] Now adding: %s (New Release Group)"
                ...
                if check_release_date is None or check_release_date == "None":
                    if not headphones.CONFIG.MB_IGNORE_AGE_MISSING:
                        logger.info(
                            "[%s] Now updating: %s (No Release Date)"
                            % (artist["artist_name"], rg["title"])
                        )
            ...
            logger.info(
                "[%s] Now adding/updating: %s (Comprehensive Force)",
                artist["artist_name"],
                rg["title"],
            )
        ...
        if new_releases != 0:
            ...
            find_hybrid_releases = myDB.select(
            ...
            try:
                ...
                logger.info(
                    "[%s] Packaging %s releases into hybrid title"
                    % (artist["artist_name"], rg["title"])
                )
            except Exception as e:
                errors = True
            ...
            album = myDB.action(
                "SELECT * from allalbums WHERE ReleaseID=?", [releaseid]
            ).fetchone()
            ...
            if rg_exists:
                ...
                if headphones.CONFIG.AUTOWANT_ALL:
                    ...
                    if (
                        helpers.is_valid_date(album["ReleaseDate"])
                        and helpers.age(album["ReleaseDate"]) < 21
                    ):
                        newValueDict["Status"] = "Wanted"
                else:
            ...
            tracks = myDB.action(
                "SELECT * from alltracks WHERE ReleaseID=?", [releaseid]
            ).fetchall()
            ...
            logger.info(
                "[%s] Seeing if we need album art for %s"
                % (artist["artist_name"], rg["title"])
            )
            try:
            ...
            if (
                not rg_exists
                and not marked_as_downloaded
                and headphones.CONFIG.AUTOWANT_ALL
            ):
                album_searches.append(rg["id"])
        else:
            if skip_log == 0:
                logger.info(
                    "[%s] No new releases, so no changes made to %s"
                    % (artist["artist_name"], rg["title"])
                )
    ...
    try:
        metacritic.update(
            artistid, artist["artist_name"], artist["releasegroups"]
        )
    except Exception as e:
...
def addReleaseById(rid, rgid=None):
    ...
    for result in results:
        ...
        logger.debug(
            "Found a cached releaseid : releasegroupid relationship: "
            + rid
            + " : "
            + rgid
        )
    if not rgid:
        ...
        if not release_dict:
            logger.info(
                "Unable to get release information for Release %s: no dict",
                rid,
            )
            if status == "Loading":
                myDB.action("DELETE FROM albums WHERE AlbumID=?", [rgid])
...
def updateFormat():
    ...
    if len(havetracks) > 0:
        ...
        logger.info(
            "Finished finding media format for %s files" % len(havetracks)
        )


def getHybridRelease(fullreleaselist):
    """Returns a dictionary of best group of tracks from the list of releases
    ...
    average_tracks = sum(
        x["trackscount"] for x in sortable_release_list
    ) / float(len(sortable_release_list))
