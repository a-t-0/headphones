...
def _verify_hmac(
    ...
    if not match:
        log.debug(
            "Verify HMAC failed: signature base string: %s", sig_base_str
        )
    return match
...
def sign_hmac_sha1(base_string, client_secret, resource_owner_secret):
    ...
    return _sign_hmac(
        "SHA-1", base_string, client_secret, resource_owner_secret
    )
...
def verify_hmac_sha256(
    ...
    return _verify_hmac(
        "SHA-256", request, client_secret, resource_owner_secret
    )
...
def sign_hmac_sha256(base_string, client_secret, resource_owner_secret):
    ...
    return _sign_hmac(
        "SHA-256", base_string, client_secret, resource_owner_secret
    )
...
def verify_hmac_sha512(
    ...
    return _verify_hmac(
        "SHA-512", request, client_secret, resource_owner_secret
    )
...
def _sign_rsa(
    hash_algorithm_name: str, sig_base_str: str, rsa_private_key: str
):
    """
