...
def main():
    ...
    if not headphones.SYS_ENCODING or headphones.SYS_ENCODING in (
        "ANSI_X3.4-1968",
        "US-ASCII",
        "ASCII",
    ):
    ...
    parser.add_argument(
        ...
        "--port",
        type=int,
        help="Force Headphones to run on a specified port",
    )
    parser.add_argument(
    ...
    parser.add_argument("--host", help="Specify a host (default - localhost)")
    ...
    logger.initLogger(
        console=not headphones.QUIET, log_dir=False, verbose=headphones.VERBOSE
    )
    ...
    if args.pidfile:
        ...
        if os.path.exists(headphones.PIDFILE):
            raise SystemExit(
                "PID file '%s' already exists. Exiting." % headphones.PIDFILE
            )
    ...
    if headphones.CONFIG.LAUNCH_BROWSER and not args.nolaunch:
        headphones.launch_browser(
            headphones.CONFIG.HTTP_HOST, http_port, headphones.CONFIG.HTTP_ROOT
        )
