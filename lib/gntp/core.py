    ...
    def set_password(self, password, encryptAlgo="MD5"):
        ...
        if not self.encryptAlgo in self.hash_algo:
            raise errors.UnsupportedError(
                'INVALID HASH "%s"' % self.encryptAlgo
            )
    ...
    def _decode_binary(self, rawIdentifier, identifier):
        ...
        if not len(data) == dataLength:
            raise errors.ParseError(
                "INVALID_DATA_LENGTH Expected: %s Received %s"
                % (dataLength, len(data))
            )
        return data
    ...
    def validate(self):
        ...
        for header in self._requiredHeaders:
            if not self.headers.get(header, False):
                raise errors.ParseError(
                    "Missing Notification Header: " + header
                )
...
class GNTPRegister(_GNTPBase):
    ...
    _requiredHeaders = ["Application-Name", "Notifications-Count"]
    ...
    def validate(self):
        ...
        for header in self._requiredHeaders:
            if not self.headers.get(header, False):
                raise errors.ParseError(
                    "Missing Registration Header: " + header
                )
...
class GNTPNotice(_GNTPBase):
    ...
    def __init__(
        self, data=None, app=None, name=None, title=None, password=None
    ):
...
class GNTPError(_GNTPBase):
    ...
    def error(self):
        return (
            self.headers.get("Error-Code", None),
