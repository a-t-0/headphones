...
class _PatternElement:
    ...
    def render(self, replacement):
        ...
        """Format this _PatternElement into string using provided substitution
        dictionary."""
...
class _Generator(_PatternElement):
    ...
    """Tagging interface for "content-generating" elements like replacement or
    optional block."""


class _Replacement(_Generator):
    """Replacement variable, eg.
...
class _OptionalBlock(_Generator):
    """Optional block will render its contents only if any _Generator in its
    scope did return non-empty result."""
...
def _append_literal(scope, text):
    ...
    """Append literal text to the scope BUT ONLY if it's not an empty
    string."""
    if len(text) == 0:
        return
...
class Warnings(Enum):
    """Pattern parsing warnings, as stored withing warnings property of Pattern
...
def _parse_pattern(pattern, warnings):
    ...
    """Parse path pattern text into list of _PatternElements, put warnings into
    the provided set."""
...
class Pattern:
    ...
    If using the same pattern repeatedly it is much more effective to
    parse the pattern into Pattern object and use it instead of parsing
    the textual pattern on each substitution. To use Pattern object for
    substitution simply call it as it was function providing dictionary
    as an argument (see __call__()).
    """
    ...
    def __call__(self, replacement):
        # type: (Mapping[str,str]) -> str
        """Execute path rendering/substitution based on replacement
        dictionary."""
