...
class BaseScheduler(six.with_metaclass(ABCMeta)):
    ...
    def start(self):
        ...
        with self._jobstores_lock:
            ...
            for job, jobstore_alias, replace_existing in self._pending_jobs:
                self._real_add_job(
                    job, jobstore_alias, replace_existing, False
                )
    ...
    def remove_job(self, job_id, jobstore=None):
        ...
        with self._jobstores_lock:
            ...
            for i, (job, jobstore_alias, replace_existing) in enumerate(
                self._pending_jobs
            ):
                if job.id == job_id:
    ...
    def _configure(self, config):
        ...
        self.timezone = (
            astimezone(config.pop("timezone", None)) or get_localzone()
        )

        # Set the job defaults
        job_defaults = config.get("job_defaults", {})
        self._job_defaults = {
            "misfire_grace_time": asint(
                job_defaults.get("misfire_grace_time", 1)
            ),
    ...
    def _real_add_job(self, job, jobstore_alias, replace_existing, wakeup):
        ...
        self._logger.info(
            'Added job "%s" to job store "%s"', job.name, jobstore_alias
        )
    ...
    def _create_plugin_instance(self, type_, alias, constructor_kwargs):
        ...
        plugin_container, class_container, base_class = {
            ...
            "jobstore": (
                self._jobstore_plugins,
                self._jobstore_classes,
                BaseJobStore,
            ),
            "executor": (
                self._executor_plugins,
        ...
        try:
            ...
            if alias in plugin_container:
                plugin_cls = class_container[alias] = plugin_container[
                    alias
                ].load()
