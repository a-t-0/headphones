...
class Torrent(object):
    ...
    def set_torrent_complete_data(self, torrent_json_response):
        if self.id != torrent_json_response["torrent"]["id"]:
            raise InvalidTorrentException(
                "Tried to update a Torrent's information from an 'artist' API call with a different id."
                + " Should be %s, got %s"
                % (self.id, torrent_json_response["id"])
            )

        self.group = self.parent_api.get_torrent_group(
            torrent_json_response["group"]["id"]
        )
        ...
        self.remaster_record_label = torrent_json_response["torrent"][
            "remasterRecordLabel"
        ]
        ...
        self.file_list = [
            ...
            for item in torrent_json_response["torrent"]["fileList"].split(
                "|||"
            )
        ]  # tuple ( filename, filesize )
        self.file_path = torrent_json_response["torrent"]["filePath"]
        self.user = self.parent_api.get_user(
            torrent_json_response["torrent"]["userId"]
        )
    ...
    def set_torrent_artist_data(self, artist_torrent_json_response):
        if self.id != artist_torrent_json_response["id"]:
            raise InvalidTorrentException(
                "Tried to update a Torrent's information from an 'artist' API call with a different id."
                + " Should be %s, got %s"
                % (self.id, artist_torrent_json_response["id"])
            )

        self.group = self.parent_api.get_torrent_group(
            artist_torrent_json_response["groupId"]
        )
        ...
        self.remaster_record_label = artist_torrent_json_response[
            "remasterRecordLabel"
        ]
    ...
    def set_torrent_group_data(self, group_torrent_json_response):
        if self.id != group_torrent_json_response["id"]:
            raise InvalidTorrentException(
                "Tried to update a Torrent's information from a 'torrentgroup' API call with a different id."
                + " Should be %s, got %s"
                % (self.id, group_torrent_json_response["id"])
            )

        self.group = self.parent_api.get_torrent_group(
            group_torrent_json_response["groupId"]
        )
        ...
        self.remaster_record_label = group_torrent_json_response[
            "remasterRecordLabel"
        ]
        self.remaster_catalogue_number = group_torrent_json_response[
        ...
        self.file_list = [
            re.match("(.+){{{(\d+)}}}", item).groups()
            for item in group_torrent_json_response["fileList"].split("|||")
        ]  # tuple ( filename, filesize )
        self.file_path = group_torrent_json_response["filePath"]
        self.user = self.parent_api.get_user(
            group_torrent_json_response["userId"]
        )

    def set_torrent_search_data(self, search_torrent_json_response):
        if self.id != search_torrent_json_response["torrentId"]:
            raise InvalidTorrentException(
                "Tried to update a Torrent's information from a 'browse'/search API call with a different id."
                + " Should be %s, got %s"
                % (self.id, search_torrent_json_response["torrentId"])
            )

        # TODO: Add conditionals to handle torrents that aren't music
        self.group = self.parent_api.get_torrent_group(
            search_torrent_json_response["groupId"]
        )
        ...
        self.remaster_catalogue_number = search_torrent_json_response[
            "remasterCatalogueNumber"
        ]
        ...
        self.free_torrent = (
            search_torrent_json_response["isFreeleech"]
            or search_torrent_json_response["isPersonalFreeleech"]
        )
        self.time = search_torrent_json_response["time"]
        self.can_use_token = search_torrent_json_response.get(
            "canUseToken", False
        )
    ...
    def set_torrent_top_10_data(self, top_10_json_response):
        if self.id != top_10_json_response["torrentId"]:
            raise InvalidTorrentException(
                "Tried to update a Torrent's information from a 'browse'/search API call with a different id."
                + " Should be %s, got %s"
                % (self.id, top_10_json_response["torrentId"])
            )

        # TODO: Add conditionals to handle torrents that aren't music
        self.group = self.parent_api.get_torrent_group(
            top_10_json_response["groupId"]
        )
        self.group.name = top_10_json_response["groupName"]
        if not self.group.music_info and top_10_json_response["artist"]:
            self.group.music_info = {
                "artists": [
                    self.parent_api.get_artist(
                        name=HTMLParser().unescape(
                            top_10_json_response["artist"]
                        )
                    )
                ]
            }
