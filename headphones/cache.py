...
class Cache:
    ...
    art, artist images, etc) and info/descriptions (album info, artist
    descrptions) to and from the cache folder. This can be called from within a
    web interface. For example, using the helper functions `getInfo(id)` and
    ...
    def get_artwork_from_cache(self, ArtistID=None, AlbumID=None):
        """Pass a musicbrainz id to this function (either ArtistID or
        AlbumID)"""
    ...
    def get_image_links(self, ArtistID=None, AlbumID=None):
        """Here we're just going to open up the last.fm url, grab the image
        links and return them Won't save any image urls, or save the artwork in
        the cache.

        Useful for search results, etc.
        """
        if ArtistID:
    ...
    def _update_cache(self):
        """Since we call the same url for both info and artwork, we'll update
