...
def validate_etags(autotags=False, debug=False):
    ...
    if status >= 200 and status <= 299:
        ...
        if debug:
            cherrypy.log(
                "If-Match conditions: %s" % repr(conditions), "TOOLS.ETAGS"
            )
        ...
        if conditions == ["*"] or etag in conditions:
            if debug:
                cherrypy.log(
                    "request.method: %s" % request.method, "TOOLS.ETAGS"
                )
...
def referer(
    ...
    try:
        ...
        if debug:
            cherrypy.log(
                "Referer %r matches %r" % (ref, pattern), "TOOLS.REFERER"
            )
        if accept == match:
            return
    except KeyError:
...
class SessionAuth(object):
    ...
    def login_screen(
        self, from_page="..", username="", error_msg="", **kwargs
    ):
        return (
            str(
                """<html><body>
