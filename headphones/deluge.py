...
def _scrubber(text):
    if scrub_logs:
        try:
            ...
            # Session cookie
            text = re.sub(
                "_session_id'\: '.*'", "_session_id': 'REMOVED'", text
            )
...
def addTorrent(link, data=None, name=None):
    try:
        ...
        if link.lower().startswith("magnet:"):
            ...
            result = {"type": "magnet", "url": link}
            retid = _add_torrent_magnet(result)

        elif link.lower().startswith("http://") or link.lower().startswith(
            "https://"
        ):
            ...
            if link.lower().startswith(tuple(url_orpheus)):
                logger.debug(
                    "Deluge: Using different User-Agent for this site"
                )
            ...
            try:
                ...
                if r.status_code == 200:
                    ...
                    logger.debug(
                        "Deluge: Trying to GET %s returned status %d"
                        % (_scrubber(link), r.status_code)
                    )
                    return False
            except Exception as e:
            ...
            if "announce" not in str(torrentfile)[:40]:
                logger.debug(
                    "Deluge: Contents of %s doesn't look like a torrent file"
                    % _scrubber(link)
                )
                return False
            if not name:
                ...
                try:
                    ...
                    name_length = int(
                        re.findall("name([0-9]*)\:.*?\:", str(torrentfile))[0]
                    )
                    logger.debug("Deluge: Getting torrent name")
                    name = re.findall("name[0-9]*\:(.*?)\:", str(torrentfile))[
                        0
                    ][:name_length]
                except Exception as e:
                    logger.debug(
            ...
            try:
                logger.debug(
                    "Deluge: Sending Deluge torrent with name %s and content [%s...]"
                    % (name, str(torrentfile)[:40])
                )
            except:
                logger.debug(
                    "Deluge: Sending Deluge torrent with problematic name and some content"
                )
            result = {"type": "torrent", "name": name, "content": torrentfile}
        ...
        if retid:
            logger.info(
                "Deluge: Torrent sent to Deluge successfully  (%s)" % retid
            )
            return retid
        else:
...
def getTorrentFolder(result):
    ...
    try:
        ...
        result["total_done"] = json.loads(response.text)["result"][
            "total_done"
        ]
...
def removeTorrent(torrentid, remove_data=False):
    ...
    try:
        logger.debug(
            "Deluge: Checking if torrent %s finished seeding" % str(torrentid)
        )
        post_data = json.dumps(
            {
                "method": "web.get_torrent_status",
                "params": [torrentid, ["name", "ratio", "state"]],
                "id": 26,
            }
        )
        ...
        try:
            ...
            logger.debug(
                'Deluge: "state" KeyError when trying to remove torrent %s'
                % str(torrentid)
            )
            return False
        ...
        if state.lower() in not_finished:
            logger.debug(
                "Deluge: Torrent %s is either queued or seeding, not removing yet"
                % str(torrentid)
            )
            return False
        else:
...
def _get_auth():
    ...
    if len(delugeweb_password) > 0:
        logger.debug(
            "Deluge: Using password %s******%s"
            % (delugeweb_password[0], delugeweb_password[-1])
        )
    ...
    if delugeweb_cert is None or delugeweb_cert.strip() == "":
        ...
        logger.debug(
            "Deluge: Using certificate %s, host is now %s"
            % (_scrubber(deluge_verify_cert), _scrubber(delugeweb_host))
        )
    ...
    post_data = json.dumps(
        {"method": "auth.login", "params": [delugeweb_password], "id": 1}
    )
    try:
        response = requests.post(
            delugeweb_url,
        ...
        try:
            logger.debug(
                "Deluge: Connection failed, let's try HTTPS just in case"
            )
            response = requests.post(
                delugeweb_url.replace("http:", "https:"),
                data=post_data.encode("utf-8"),
                cookies=delugeweb_auth,
                verify=deluge_verify_cert,
                headers=headers,
            ...
            logger.error(
                "Deluge: Switching to HTTPS, but certificate won't be verified because NO CERTIFICATE WAS CONFIGURED!"
            )
    ...
    logger.debug(
        "Deluge: Authentication result: %s, Error: %s" % (auth, auth_error)
    )
    delugeweb_auth = response.cookies
    logger.debug(
        "Deluge: Authentication cookies: %s"
        % _scrubber(str(delugeweb_auth.get_dict()))
    )
    ...
    logger.debug(
        "Deluge: Connection result: %s, Error: %s"
        % (connected, connected_error)
    )

    if not connected:
        post_data = json.dumps(
            {"method": "web.get_hosts", "params": [], "id": 11}
        )
        try:
            response = requests.post(
                delugeweb_url,
        ...
        post_data = json.dumps(
            {"method": "web.connected", "params": [], "id": 10}
        )
...
def _add_torrent_magnet(result):
    ...
    try:
        ...
        logger.debug(
            "Deluge: Response was %s" % str(json.loads(response.text))
        )
...
def setTorrentLabel(result):
    ...
    if label:
        ...
        post_data = json.dumps(
            {"method": "label.get_labels", "params": [], "id": 3}
        )
        response = requests.post(
            delugeweb_url,
            data=post_data.encode("utf-8"),
        ...
        if labels is not None:
            if label not in labels:
                try:
                    ...
                    post_data = json.dumps(
                        {"method": "label.add", "params": [label], "id": 4}
                    )
                    response = requests.post(
                        delugeweb_url,
                        data=post_data.encode("utf-8"),
...
def setTorrentPath(result):
    ...
    try:
        ...
        logger.error(
            "Deluge: Setting torrent move-to directory failed: %s" % str(e)
        )
        formatted_lines = traceback.format_exc().splitlines()
        logger.error("; ".join(formatted_lines))
        return None
