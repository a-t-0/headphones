...
def listener():
    """Wrapper that create a QueueListener, starts it and automatically stops
    it.

    To be used in a with statement in the main process, for
    multiprocessing.
    """
    ...
    if queue is None:
        try:
            ...
            if e.errno == errno.EACCES:
                logger.warning(
                    "Multiprocess logging disabled, because "
...
def initMultiprocessing():
    ...
    This should only be called inside a multiprocessing worker process,
    since it changes the logger completely.
    """
    ...
    * StreamHandler: for console (if console)
    ...
    if log_dir:
        ...
        file_handler = handlers.RotatingFileHandler(
            filename, maxBytes=MAX_SIZE, backupCount=MAX_FILES, encoding="utf8"
        )
...
def initHooks(
    global_exceptions=True, thread_exceptions=True, pass_original=True
):
